...
class TestMCPToolSupport(unittest.TestCase):
    """Tests for Requirement 6: MCP Tool Support"""
    ...
    def setUp(self):
        ...
        for pos in range(1, 6):
            ...
            with open(filepath, 'w') as f:
                # Create simple frequency files
                f.write('1000 s\n900 i\n800 o\n700 u\n600 e\n')
        
        # Create test word file
        words_file = os.path.join(self.tmpdir, 'wordle-words.txt')
        test_words = [
            'saint', 'guise', 'poise', 'noise', 'moise', 'hoise',
            'brisk', 'chips', 'clips', 'prism', 'slain', 'stain'
        ]
        with open(words_file, 'w') as f:
            f.write('\n'.join(test_words))
        
        self.solver = WordleSolver(frequency_dir=self.tmpdir, words_file=words_file)
    ...
    def test_process_feedback_returns_candidates_and_suggestions(self):
        """
        Test Case 6.2.3.1: process_feedback returns dictionary with candidates and suggestions
        
        Requirement 6.2.3: process_feedback returns dictionary with candidates and suggestions
        
        Given: Valid feedback parameters
        When: process_feedback is called
        Then: Returns dictionary with candidates list and suggestions list
        """
        result = self.solver.process_feedback(
            guess="saint",
            greens="..i..",
            yellows="s....",
            greys=["a", "n", "t"]
    ...
    def test_process_feedback_with_saint_example(self):
        """
        Test Case 6.4.1.1: process_feedback works with predefined test case
        
        Requirement 6.4.1: Tool tested using predefined test case
        Requirement 6.4.2: Expected output contains valid candidate words and suggestions
        
        Given: guess="saint", greens="..i..", yellows="s....", greys=["a", "n", "t"]
        When: process_feedback is called
        Then: Returns valid candidates and suggestions (e.g., ["GUISE", "POISE"])
        """
        result = self.solver.process_feedback(
            guess="saint",
            greens="..i..",
        ...
        self.assertGreater(len(result["candidates"]), 0, "Should have candidate words")
        ...
        if result["suggestions"]:
            for suggestion in result["suggestions"]:
                ...
                self.assertIsInstance(suggestion["word"], str)
                self.assertIsInstance(suggestion["score"], int)
        
        # Check that vowel-rich words like GUISE and POISE are in candidates
        candidate_words_upper = [w.upper() for w in result["candidates"]]
        # Note: Actual candidates depend on filtering logic, but should contain valid words
        self.assertTrue(
            all(isinstance(w, str) and len(w) == 5 for w in result["candidates"]),
            "All candidates should be 5-letter strings"
        )
    
    def test_process_feedback_handles_empty_greens(self):
        """
        Test Case 6.2.2.1: process_feedback handles empty greens string
        
        Requirement 6.2.2: process_feedback takes greens parameter
        
        Given: Empty greens string
        When: process_feedback is called
        Then: Processes correctly without errors
        """
        result = self.solver.process_feedback(
            guess="saint",
            greens=".....",
    ...
    def test_process_feedback_handles_empty_yellows(self):
        """
        Test Case 6.2.2.2: process_feedback handles empty yellows string
        
        Requirement 6.2.2: process_feedback takes yellows parameter
        
        Given: Empty yellows string
        When: process_feedback is called
        Then: Processes correctly without errors
        """
        result = self.solver.process_feedback(
    ...
    def test_process_feedback_handles_empty_greys(self):
        """
        Test Case 6.2.2.3: process_feedback handles empty greys list
        
        Requirement 6.2.2: process_feedback takes greys parameter
        
        Given: Empty greys list
        When: process_feedback is called
        Then: Processes correctly without errors
        """
        result = self.solver.process_feedback(
    ...
    def test_process_feedback_suggestions_have_word_and_score(self):
        """
        Test Case 6.1.2.1: Suggestions contain word and score
        
        Requirement 6.1.2: Tool returns suggestions with word-score pairs
        
        Given: Feedback that yields suggestions
        When: process_feedback is called
        Then: Each suggestion has "word" and "score" keys
        """
        result = self.solver.process_feedback(
            guess="saint",
            greens=".....",
        ...
        if result["suggestions"]:
            for suggestion in result["suggestions"]:
                ...
                self.assertIsInstance(suggestion["word"], str)
                self.assertIsInstance(suggestion["score"], int)
                self.assertEqual(len(suggestion["word"]), 5)


class TestFastAPIEndpoint(unittest.TestCase):
    """Tests for Requirement 6.5: FastAPI HTTP API Endpoint"""
    ...
    def test_fastapi_endpoint_exists(self):
        """
        Test Case 6.5.1.1: FastAPI endpoint /process_feedback exists
        
        Requirement 6.5: Expose HTTP API endpoint via FastAPI
        Requirement 6.5.1: POST endpoint at /process_feedback
        
        Given: FastAPI app is imported
        When: Checking for /process_feedback endpoint
        Then: Endpoint exists and accepts POST requests
        """
        try:
            from mcp.api import app
            # Check that app has routes
            routes = [route.path for route in app.routes]
            self.assertIn("/process_feedback", routes)
            
            # Find the process_feedback route
            process_feedback_route = None
            for route in app.routes:
                if route.path == "/process_feedback":
                    process_feedback_route = route
                    break
            
            self.assertIsNotNone(process_feedback_route, "process_feedback route should exist")
            # Verify it's a POST endpoint
            self.assertIn("POST", [method for method in process_feedback_route.methods])
        except ImportError:
            self.skipTest("FastAPI not installed - skipping FastAPI tests")
    
    def test_fastapi_request_response_format(self):
        """
        Test Case 6.5.2.1: FastAPI endpoint returns correct JSON response format
        
        Requirement 6.5.2: Return JSON response with candidates and suggestions
        
        Given: Valid request to /process_feedback
        When: Endpoint processes request
        Then: Returns JSON with candidates list and suggestions list
        """
        try:
            from mcp.api import ProcessFeedbackRequest, ProcessFeedbackResponse
            from wordle_solver import WordleSolver
            
            # Create solver with test data
            solver = WordleSolver(frequency_dir=self.tmpdir, words_file=os.path.join(self.tmpdir, 'wordle-words.txt'))
            ...
            # Process feedback directly (simulating endpoint call)
            ...
            # Verify response structure matches ProcessFeedbackResponse model
            self.assertIn("candidates", result)
            self.assertIn("suggestions", result)
            self.assertIsInstance(result["candidates"], list)
            self.assertIsInstance(result["suggestions"], list)
    ...
    def test_fastapi_health_endpoint(self):
        """
        Test Case 6.5.3.1: FastAPI app has health check endpoint
        
        Requirement 6.5.3: Runnable as standalone server
        
        Given: FastAPI app
        When: Checking for health endpoint
        Then: /health endpoint exists
        """
        try:
            from mcp.api import app
            routes = [route.path for route in app.routes]
