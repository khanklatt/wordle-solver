...
### MCP Tool Support
- 6.1: APPROVED, MVP – System shall expose the Wordle solver as an MCP tool that can be invoked with feedback and return a list of candidate words and suggested guesses.
- 6.1.1: APPROVED, MVP – The MCP tool should receive the following parameters:
guess (str): The guessed word.
greens (str): Feedback indicating correct letters (e.g., '..i..').
yellows (str): Feedback indicating incorrect letter positions (e.g., 's....').
greys (list): A list of letters that are excluded (e.g., ['a', 'n', 't']).
- 6.1.2: APPROVED, MVP – The tool shall process these inputs and return the following:
candidates (list of str): Possible candidate words.
suggestions (list of dicts): Suggested words with associated scores (e.g., {"word": "GUISE", "score": 19}).
- 6.2: APPROVED, MVP – The system shall refactor the existing solver logic in wordle_solver.py into a clean, callable function within the WordleSolver class.
- 6.2.1: APPROVED, MVP – The refactored function shall be named process_feedback.
- 6.2.2: APPROVED, MVP – process_feedback shall take the following parameters:
guess (str)
greens (str)
yellows (str)
greys (list of str)
- 6.2.3: APPROVED, MVP – process_feedback shall return a dictionary containing:
candidates (list of str)
suggestions (list of dicts with word-score pairs).
- 6.3: APPROVED, MVP – System shall implement wordle_mcp.py to expose the WordleSolver as an MCP tool.
- 6.3.1: APPROVED, MVP – The function in wordle_mcp.py shall be named suggest_guess.
- 6.3.2: APPROVED, MVP – suggest_guess shall call WordleSolver.process_feedback and return the results in a valid MCP format.
- 6.4: APPROVED, MVP – System shall ensure that the MCP tool can be tested by simulating feedback and verifying the output.
- 6.4.1: APPROVED, MVP – The tool shall be tested using predefined test cases (e.g., for the guess "saint", with greens "..i..", yellows "s....", and greys ["a", "n", "t"]).
- 6.4.2: APPROVED, MVP – Expected output for the test case shall contain valid candidate words and suggestions (e.g., ["GUISE", "POISE"]).
- 6.5: APPROVED, MVP – System shall expose an HTTP API endpoint via FastAPI to enable remote access to the Wordle solver as an MCP tool.
- 6.5.1: APPROVED, MVP – The API shall provide a POST endpoint at /process_feedback that accepts JSON request body with the following fields:
guess (str): The guessed word.
greens (str): Feedback indicating correct letters (e.g., '..i..').
yellows (str): Feedback indicating incorrect letter positions (e.g., 's....').
greys (list of str): A list of letters that are excluded (e.g., ['a', 'n', 't']).
- 6.5.2: APPROVED, MVP – The endpoint shall return JSON response with the following structure:
candidates (list of str): Possible candidate words.
